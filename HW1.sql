...
SELECT s.first_name, s.last_name, SUM(p.amount) AS 'TOTAL' FROM staff s LEFT JOIN payment p  ON s.staff_id = p.staff_id GROUP BY s.first_name, s.last_name;
...
SELECT last_name, first_name, SUM(amount) FROM payment p INNER JOIN customer c 
